...
contract SupplyChain {
  ...
  modifier checkCaller(uint _itemId, State _state) {
    require((_state == State.Sold && items[_itemId].seller == msg.sender) || (_state == State.Shipped && items[_itemId].buyer == msg.sender));
    _;
  }
  ...
  modifier checkValue(uint _itemId) {
    require(myfee <= msg.value || (items[_itemId].seller != address(0) && items[_itemId].price + myfee >= msg.value));
    _;
  }
  ...
  function addItem(string calldata _name, uint _price) checkValue(0) external payable {
    ...
    itemIdCount++;
    msg.sender.transfer(msg.value - myfee);

    emit StateChangeLog(itemIdCount-1, item.name, item.price, item.state, item.seller, item.buyer);
